...
def sort(array):
    ...
    for _pass in range(
        lengthOfData
    ):  # Iterate through our array, these will be our "passes"

        for value in range(
            lengthOfData - _pass - 1
        ):  # Iterate through the unsorted portion of the array, excluding the last _pass elements as they are already sorted.

            if (
                array[value] > array[value + 1]
            ):  # Compare two values and if the first one is larger than the second:
                array[value], array[value + 1] = (
                    array[value + 1],
                    array[value],
