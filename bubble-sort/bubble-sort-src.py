...
for _pass in range(lengthOfData): # Iterate through our array, these will be our "passes"
    ...
    print("\nPass:", str(_pass)) # Print out the current pass
    ...
    for value in range(lengthOfData-_pass-1): # Iterate through the unsorted portion of the array, excluding the last _pass elements as they are already sorted.
        ...
        if data[value] > data[value+1]: # Compare two values and if the first one is larger than the second:
            data[value], data[value+1] = data[value+1], data[value] # Swap the first and second values

            print(f"Swapped {str(data[value])} and {str(data[value+1])}") # Print which values have been swapped
